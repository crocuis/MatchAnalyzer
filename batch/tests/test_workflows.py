...
def test_sync_prediction_checkpoints_workflow_targets_due_matches_and_daily_pick_dates() -> None:
    ...
    assert "MATCH_ANALYZER_DISABLE_DAILY_PICK_TRACKING_SYNC=1" in workflow
    assert "MATCH_ANALYZER_OMIT_PREDICTION_OUTPUT_PAYLOAD=1" not in workflow
...
def test_deploy_production_workflow_documents_required_production_secrets() -> None:
    ...
    assert "CLOUDFLARE_HYPERDRIVE_SYNC_ENABLED" in workflow
    assert "if: env.CLOUDFLARE_HYPERDRIVE_SYNC_ENABLED == 'true'" in workflow
    assert "MATCH_ANALYZER_TRUST_HYPERDRIVE_FRESH" in workflow
    assert "wrangler secret put MATCH_ANALYZER_TRUST_HYPERDRIVE_FRESH" in workflow
    assert "match-analyzer-deploy-smoke/1.0" in workflow
