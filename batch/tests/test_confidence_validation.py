...
def test_high_confidence_eligibility_reports_best_failing_broader_segment():
    records = [
        _record(index, correct=True)
        for index in range(10)
    ]
    records.extend(
        {
            **_record(index + 10, correct=index < 30),
            "league_or_sport": "serie-a",
        }
        for index in range(50)
    )
    ...
    assert decision["high_confidence_eligible"] is False
    assert decision["confidence_reliability"] == "below_target_hit_rate"
    assert decision["validation_metadata"]["sample_count"] == 60
    assert decision["validation_metadata"]["hit_rate"] == 0.6667
    assert decision["validation_metadata"]["league_or_sport"] == "all"
    assert decision["validation_metadata"]["validation_scope"] == "global_confidence_implied"
