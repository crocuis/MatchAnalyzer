...
def test_sync_daily_picks_reports_betman_value_pick_invalid_for_unknown_betman_pick() -> None:
    run, items = sync_daily_picks_for_date(
        pick_date="2026-04-24",
        matches=[
            {
        ...
        predictions=[
            {
                ...
                "main_recommendation_recommended": True,
                "main_recommendation_no_bet_reason": None,
                "value_recommendation_pick": "HOME_OR_DRAW",
                "value_recommendation_recommended": True,
                "value_recommendation_market_source": "betman_moneyline_3way",
                "summary_payload": {
                    "betman_market_available": True,
    ...
    assert run["metadata"]["recommended_count"] == 0
    assert run["metadata"]["held_count"] == 0
    assert run["metadata"]["hold_reason_counts"] == {"betman_value_pick_invalid": 1}


def test_sync_daily_picks_reports_betman_market_missing_for_non_betman_value_source_without_market_flag() -> None:
    run, items = sync_daily_picks_for_date(
        pick_date="2026-04-24",
        matches=[
            {
...
def test_sync_daily_picks_prefers_t24_checkpoint_over_later_pre_match_rows() -> None:
    _run, items = sync_daily_picks_for_date(
        pick_date="2026-04-24",
        matches=[
            {
        ...
        predictions=[
            {
                ...
                "summary_payload": {
                    ...
                    "moneyline_signal_score": 4.0,
                    "source_agreement_ratio": 0.67,
                    "feature_context": {"external_rating_available": True},
                    "validation_metadata": {
                        "sample_count": 30,
                        "hit_rate": 0.69,
                        "wilson_lower_bound": 0.5,
            ...
            },
            {
                ...
                "main_recommendation_pick": "HOME",
                "main_recommendation_confidence": 0.45,
                "main_recommendation_recommended": False,
                "main_recommendation_no_bet_reason": "below_high_confidence_threshold",
                "summary_payload": {
                    "base_model_source": "trained_baseline_poisson_blend",
                    "max_abs_divergence": 0.04,
                    "moneyline_signal_score": 0.0,
                    "source_agreement_ratio": 0.3,
                    "validation_metadata": {
                        "sample_count": 30,
                        "hit_rate": 0.69,
                        "wilson_lower_bound": 0.5,
