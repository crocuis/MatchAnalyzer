...
describe("MatchTable", () => {
  it("anchors the upcoming preview window to the first upcoming fixture", () => {
    ...
    try {
      ...
      expect(screen.getByRole("button", { name: "Show 1 later matches" })).toBeInTheDocument();
    } finally {
      nowSpy.mockRestore();
    }
  });
  ...
  it("prefetches recent results without exposing later upcoming matches", () => {
    const observe = vi.fn();
    ...
    try {
      ...
      expect(screen.getByRole("button", { name: "Show 2 later matches" })).toBeInTheDocument();
  ...
  it("continues to offer loading more recent results after recent matches become visible", () => {
    const observe = vi.fn();
    const disconnect = vi.fn();
    const onLoadMore = vi.fn();
    const nowSpy = vi.spyOn(Date, "now").mockReturnValue(Date.parse("2026-04-24T00:00:00Z"));
    ...
    try {
      ...
      expect(screen.getByRole("button", { name: /load more matches/i })).toBeInTheDocument();
