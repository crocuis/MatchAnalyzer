...
4. Neon 개발 브랜치의 연결 문자열을 `.env.local`, `apps/api/.dev.vars`, `batch/.env.local`에 `DATABASE_URL`로 저장한다.
   ...
   API Worker를 Hyperdrive 로컬 바인딩 형태로 검증하려면 `CLOUDFLARE_HYPERDRIVE_ID`와 `CLOUDFLARE_HYPERDRIVE_LOCAL_CONNECTION_STRING_HYPERDRIVE`를 export한 뒤 `npm --workspace apps/api run dev:hyperdrive`를 실행한다. fresh Hyperdrive 경로까지 검증하려면 `CLOUDFLARE_HYPERDRIVE_FRESH_ID`와 `CLOUDFLARE_HYPERDRIVE_LOCAL_CONNECTION_STRING_HYPERDRIVE_FRESH`도 추가한다. 로컬 Hyperdrive 바인딩은 실제 캐시를 적용하지 않는다.
...
8. GitHub Actions와 배포 환경은 Neon 연결 문자열을 GitHub secret `NEON_DATABASE_URL` 또는 `DATABASE_URL`로 받는다.
   ...
   - `CLOUDFLARE_HYPERDRIVE_FRESH_ID` (선택 production variable, query cache disabled)
