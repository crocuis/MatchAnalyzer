...
- Postgres/Neon 읽기 모델을 조회하는 Cloudflare Workers API
- 경기 수집, 마켓 수집, 예측 생성, 사후 리뷰 생성 배치 파이프라인
- Postgres 스키마/시드와 GitHub Actions 기반 샘플 자동화
...
4. API가 Postgres/Neon의 읽기 모델을 조회한다.
...
- API: Cloudflare Workers, Hono, Neon serverless driver
...
1. Neon 개발 브랜치 연결 문자열을 `DATABASE_URL`로 설정한다.
2. 새 DB를 직접 준비할 때만 `supabase/migrations/202604180001_initial_schema.sql`과 `supabase/seed.sql`을 적용한다.
...
원격 DB egress와 upsert 비용 없이 예측 실험을 반복하려면, 먼저 필요한 읽기 테이블을 로컬 JSON으로 export한다.
...
이후 예측 생성과 raw signal 평가는 같은 디렉터리를 지정하면 원격 DB 대신 로컬 파일을 읽고 쓴다.
...
python3 -m batch.src.jobs.report_daily_pick_segment_quality_job --include-segments
```
...
- `CLOUDFLARE_HYPERDRIVE_FRESH_ID` (production variable, query cache disabled)
...
- 현재 GitHub Actions 자동화는 Neon `DATABASE_URL` 기반 운영 연결을 전제로 하며, SQL migration은 배포 job에서 ledger 기반으로 한 번씩 적용한다.
