...
description: |
  ...
  Don't use when: user asks about NFL, college football, NBA, WNBA, college basketball, NHL, MLB, tennis, golf, Formula 1, or betting odds. Don't use for live scores. Limits: `get_season_leaders` and `get_missing_players` are Premier League only; `get_event_xg` is top-5 leagues only.
license: MIT
metadata:
