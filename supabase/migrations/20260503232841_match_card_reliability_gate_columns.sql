alter table public.match_card_projection_cache
  add column if not exists confidence_reliability text;

alter table public.match_card_projection_cache
  add column if not exists high_confidence_eligible boolean;
...
begin
  ...
  with representative_predictions as (
    select
      ...
      coalesce(
        predictions.summary_payload ->> 'confidence_reliability',
        predictions.summary_payload ->> 'confidenceReliability'
      ) as confidence_reliability,
      case lower(coalesce(
        predictions.summary_payload ->> 'high_confidence_eligible',
        predictions.summary_payload ->> 'highConfidenceEligible'
      ))
        when 'true' then true
  ...
  select
    ...
    representative_predictions.confidence_reliability,
    representative_predictions.high_confidence_eligible,
    coalesce(
...
select
  ...
  match_card_projection_cache.confidence_reliability,
  match_card_projection_cache.high_confidence_eligible
...
select
  ...
  match_cards.confidence_reliability,
  match_cards.high_confidence_eligible
from match_cards;
